---
name: claude-init-gate
description: "Bootstrap a repository's deterministic, LLM-debuggable quality gate before feature work: inspect stack, code structure, and command execution risk; plan and scaffold check-only make check targets, pre-commit, and CI with approval. Use for Claude Code when asked to set up repo quality gates, first-repo bootstrap, LLM-friendly codebase setup, gate 깔아줘, scaffold checks, or add a new language/stack gate; not for git init alone."
---
...
Use this Claude Code-specific skill to create or verify a repository's deterministic quality gate before feature work begins. It creates **LLM-debuggable enforcement infrastructure** — reviewed tools, pinned or lockfile-backed config, a runnable `make check`, and explicit code-structure guidance — not project narrative. It is a quality-gate bootstrap, not a general `git init` replacement.
...
- Do not write narrative `CLAUDE.md` or `AGENTS.md` content. If a persistent Claude Code operating rule is needed, add at most one approved operational line or hand off to `write-agents-md` after the gate is verified.
...
6. **Claude Code self-correct loop.** After approved edits and approved command execution, run the canonical check path, read redacted failures, fix only the approved owned scope, and repeat until green or until a real blocker, suspicious command behavior, legacy violations outside scope, nondeterminism, or permission-sensitive action requires user input. If wiring Claude Code hooks/settings, first confirm the current hook mechanism from available docs or user-provided config; do not invent hook event names. Never weaken the gate to make it pass.
